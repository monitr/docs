...
<Icon iconName="SkypeCheck"/> ID needs to be unique
<Icon iconName="SkypeCheck"/> Rank needs to be an integer and unique for the report (BS or PL)
<Icon iconName="SkypeCheck"/> Parent any PartentID used should exist in ID
<Icon iconName="SkypeCheck"/> The value of Parent cannot be the same as ID
<Icon iconName="SkypeCheck"/> The Report of the Parent should be the same as the Report of the line itself
<Icon iconName="SkypeCheck"/> The Report needs to be BS or PL
<Icon iconName="SkypeCheck"/> Top is 0 or 1
<Icon iconName="SkypeCheck"/> A CF_ID should only be entered for lines without children (the ID of the line is not used in Parent) and to ID’s in the table Cash Flow without children
